...
const Columns = (
  ...
  },
  {
    ...
    width: "250px",
    fixed: "right",
    render: (_: any, record: SupportType) => {
      ...
      return (
        <div className="dk-flex dk-gap-3 dk-text-[#1677ff]">
          <EditRecord
            onInit={() => {
              edit(record, record.SupportTypeId?.toString() || "");
            }}
            Save={() => save(record?.SupportTypeId || "")}
            Cancel={cancel}
            Form={form}
          />
          <Popconfirm
            title={"Bạn có chắc chắn muốn xóa kiểu hỗ trợ này ?"}
            onConfirm={async () => {
              const result = await handleDelete(
                record.SupportTypeId,
                customerTypes,
                setCustomerTypes
              );
              setPopup({
